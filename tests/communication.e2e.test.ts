...
describe("StudySwap Backend - Communication Hard Tests", () => {
  ...
  it("✅ Register two students", async () => {
    const cRes = await request(app).get("/api/countries");
    const countryId = cRes.body.data.countries.length > 0 ? cRes.body.data.countries[0].id : "00000000-0000-0000-0000-000000000000";

    // Student 1
    let res = await request(app).post("/api/auth/send-otp").send({ email: s1Email });
    expect(res.status).toBe(200);
    let otp = await redis.get(`otp:${s1Email}`);
    res = await request(app).post("/api/auth/verify-otp").send({ email: s1Email, otp });
    s1Token = res.body.data.token;
    s1Id = res.body.data.user.id;

    res = await request(app).patch("/api/onboarding/profile").set("Authorization", `Bearer ${s1Token}`).send({
      name: "Student 1",
      city: "City A",
      state: "State A",
      countryId,
    ...
    // Student 2
    res = await request(app).post("/api/auth/send-otp").send({ email: s2Email });
    expect(res.status).toBe(200);
    otp = await redis.get(`otp:${s2Email}`);
    res = await request(app).post("/api/auth/verify-otp").send({ email: s2Email, otp });
    s2Token = res.body.data.token;
    s2Id = res.body.data.user.id;

    res = await request(app).patch("/api/onboarding/profile").set("Authorization", `Bearer ${s2Token}`).send({
      name: "Student 2",
      city: "City B",
      state: "State B",
      countryId,
  ...
  it("✅ Both can fetch conversations", async () => {
    const res1 = await request(app).get("/api/communication/conversations").set("Authorization", `Bearer ${s1Token}`);
    expect(res1.body.data.length).toBeGreaterThan(0);
    conversationId = res1.body.data[0].conversationId;

    const res2 = await request(app).get("/api/communication/conversations").set("Authorization", `Bearer ${s2Token}`);
